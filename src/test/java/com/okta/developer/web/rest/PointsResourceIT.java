...
class PointsResourceIT {
    ...
    void createPoints() throws Exception {
        ...
        restPointsMockMvc
            .perform(
                post("/api/points").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(points))
            )
    ...
    void getAllPoints() throws Exception {
        ...
        restPointsMockMvc
            .perform(get("/api/points?sort=id,desc"))
    ...
    void getPoints() throws Exception {
        ...
        // Get the points
        restPointsMockMvc
            .perform(get("/api/points/{id}", points.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    @Test
    @Transactional
    void updateNonExistingPoints() throws Exception {
        int databaseSizeBeforeUpdate = pointsRepository.findAll().size();

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restPointsMockMvc
            .perform(
                put("/api/points").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(points))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void partialUpdatePointsShouldThrown() throws Exception {
        // Update the points without id should throw
        Points partialUpdatedPoints = new Points();
    ...
    void deletePoints() throws Exception {
        ...
        // Delete the points
        restPointsMockMvc
            .perform(delete("/api/points/{id}", points.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
    ...
    void searchPoints() throws Exception {
        ...
        // Search the points
        restPointsMockMvc
            .perform(get("/api/_search/points?query=id:" + points.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(points.getId().intValue())))
            .andExpect(jsonPath("$.[*].date").value(hasItem(DEFAULT_DATE.toString())))
