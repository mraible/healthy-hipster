...
public class PreferencesResource {
    ...
    /**
     * {@code PATCH  /preferences} : Updates given fields of an existing preferences.
     *
     * @param preferences the preferences to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated preferences,
     * or with status {@code 400 (Bad Request)} if the preferences is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/preferences", consumes = "application/merge-patch+json")
    public ResponseEntity<Preferences> partialUpdatePreferences(@NotNull @RequestBody Preferences preferences) throws URISyntaxException {
        log.debug("REST request to update Preferences partially : {}", preferences);
        if (preferences.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
