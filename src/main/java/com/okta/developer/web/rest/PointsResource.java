...
public class PointsResource {
    ...
    /**
     * {@code PATCH  /points} : Updates given fields of an existing points.
     *
     * @param points the points to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated points,
     * or with status {@code 400 (Bad Request)} if the points is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/points", consumes = "application/merge-patch+json")
    public ResponseEntity<Points> partialUpdatePoints(@NotNull @RequestBody Points points) throws URISyntaxException {
        log.debug("REST request to update Points partially : {}", points);
        if (points.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
