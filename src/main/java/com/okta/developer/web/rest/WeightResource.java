...
public class WeightResource {
    ...
    /**
     * {@code PATCH  /weights} : Updates given fields of an existing weight.
     *
     * @param weight the weight to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated weight,
     * or with status {@code 400 (Bad Request)} if the weight is not valid,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/weights", consumes = "application/merge-patch+json")
    public ResponseEntity<Weight> partialUpdateWeight(@NotNull @RequestBody Weight weight) throws URISyntaxException {
        log.debug("REST request to update Weight partially : {}", weight);
        if (weight.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
